...
class Vendor(object):
	def __init__(self, index, timestamp, vendor_type):
...
class Cluster(object):
	...
	def add_vendor(self, vendor):
		if self.vendor_distribution is None:
			self.vendor_distribution = np.copy(vendor.type)
		else:
			self.vendor_distribution += vendor.type
...
def log_dirichlet_categorical_distribution(vendor_type_distribution, event_type, priors):
	''' compute the log dirichlet categorical distribution
		@param:
			1. vendor_type_distribution: 1-D numpy array, including event type_distribution
			2. event_type: one-hot vector with non-zero index as activity type.
			3. priors: 1-d np.array
		@rtype: float
	'''
	priors_sum = np.sum(priors)
	mode_type_count = np.sum(vendor_type_distribution)

	log_prob = 0
	log_prob += np.log(priors[event_type.astype(bool)][0] + vendor_type_distribution[event_type.astype(bool)][0])
